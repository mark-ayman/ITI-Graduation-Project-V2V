...
void MUSART_vTransmitNumAsynch(USART_MemMap_t *USARTx,u8 Copy_u8Number){
	u8 Copy_Num=Copy_u8Number;
	u8 n=0;
	//3add el arkam eli fe el Number
	while(Copy_Num!=0){
		Copy_Num/=10;
		n++;
	}
	//creae array
	u8 Num[n];
	//u8 *Local_pu8Number=&Num;
	u8 Local_u8Counter=0;
	MUSART_vTransmitByteAsynch(USARTx,'M');
	if(Copy_u8Number>9)
	{
		while(Copy_u8Number!=0)
		{
			Num[Local_u8Counter]=(Copy_u8Number%10);
			Copy_u8Number=Copy_u8Number/10;
			Local_u8Counter++;
		}
		Local_u8Counter=Local_u8Counter-1;;
		while(Local_u8Counter!=255)
		{
			MUSART_vTransmitByteAsynch(USARTx,(Num[Local_u8Counter]+ 48) );
			Local_u8Counter--;
		}
	}
	else{
		MUSART_vTransmitByteAsynch(USARTx,Copy_u8Number+48);
	}
	MUSART_vTransmitByteAsynch(USART2,'N');
}

void MUSART_vTransmitNumSynch(USART_MemMap_t *USARTx,u8 Copy_u8Number){
	u8 Copy_Num=Copy_u8Number;
	u8 n=0;
	//3add el arkam eli fe el Number
	while(Copy_Num!=0){
	...
	if(Copy_u8Number>9)
	{
		...
		while(Local_u8Counter!=255)
		{
			MUSART_vTransmitByteSynch(USARTx,(Num[Local_u8Counter]+ 48) );
			Local_u8Counter--;
		}
	}
	else{
		MUSART_vTransmitByteSynch(USARTx,Copy_u8Number+48);
	}
}
