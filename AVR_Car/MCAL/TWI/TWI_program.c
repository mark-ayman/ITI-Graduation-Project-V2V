...
TWI_ErrorStatus_t TWI_ErrorStatusMasterReadDataByteWithACK(u8 * Copy_pu8ReceivedByte)
{
	...
	/*wait until the operation finishes and the flag is raised again*/
	while((GET_BIT(TWCR, TWCR_TWINT) == 0 )&&(TimeOut_Counter<=1000)){
			TimeOut_Counter++;
	};
	...
	else if((GET_BIT(TWCR, TWCR_TWINT) == 0 )&&(TimeOut_Counter==1000)){
...
TWI_ErrorStatus_t TWI_ErrorStatusMasterReadDataByteWithNOTACK(u8 * Copy_pu8ReceivedByte)
{
	u32 TimeOut_Counter=0;
	TWI_ErrorStatus_t Local_ErrorStatus = NoError;
	...
	{
		Local_ErrorStatus = MasterReadByteWithACKError*2;
	}
	else if((GET_BIT(TWCR, TWCR_TWINT) == 0 )&&(TimeOut_Counter==1000)){
